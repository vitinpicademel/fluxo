...
class FinancialService {
    ...
    static async generateFinancialProjections(projectId, totalUnits, averageUnitValue, salesData) {
        ...
        for (const sales of salesData) {
            const salesValue = this.calculateMonthlySalesValue(totalVGV, totalUnits, sales.unitsSold);
            const paymentTerms = this.calculatePaymentTerms(salesValue);
            allParcels.push(paymentTerms.monthlyParcel);
            const receivable = this.calculateMonthlyReceivables(paymentTerms.entryValue, paymentTerms.monthlyParcel, allParcels.slice(0, -1) // Todas as parcelas anteriores
            );
